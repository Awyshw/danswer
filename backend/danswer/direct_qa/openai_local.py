...
def _handle_openai_chat_exceptions_wrapper(openai_call: F, messages: list[dict, Any]) -> F:
    @wraps(openai_call)
    def wrapped_call(*args: list[Any], **kwargs: dict[str, Any]) -> Any:
        try:
            # if streamed, the call returns a generator
            ...
            logger.exception(f"OpenAI API timed out for query: {messages}")
            raise
        except Exception:
            logger.exception(f"Unexpected error with OpenAI API for messages: {messages}")
            raise
...
class OpenAILocalCompletionQA(OpenAIQAModel):
    def __init__(
        self,
...
class OpenAILocalChatCompletionQA(OpenAIQAModel):
    def __init__(
        self,
    ...
    def answer_question(
        ...
        openai_call = _handle_openai_chat_exceptions_wrapper(
            openai_call=openai.ChatCompletion.create,
            messages=filled_prompt,
        )
        response = openai_call(
            **_build_openai_settings(
                api_key=_ensure_openai_api_key(self.api_key),
        ...
        model_output = cast(str, response["choices"][0]["message"]["content"]).strip()
        logger.info("OpenAI Token Usage: " + str(response["usage"]).replace("\n", ""))
        logger.debug(model_output)
